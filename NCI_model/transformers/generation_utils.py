...
class GenerationMixin:
    ...
    def generate(
        ...
        **model_kwargs
        ...
        decode_embedding = model_kwargs.pop('decode_embedding', None)
        decode_vocab_size = model_kwargs.pop('decode_vocab_size', None)
        decode_tree = model_kwargs.pop('decode_tree', None)
        decoder_index = model_kwargs.pop('decoder_index', None)
        ...
        if num_beams > 1:
            output = self._generate_beam_search(
                ...
                num_beams=num_beams,
                vocab_size=decode_vocab_size if decode_embedding else vocab_size,
    ...
    def _generate_beam_search(
        ...
        while cur_len < max_length:
            ...
            outputs = self(**model_inputs, return_dict=True, decoder_index=decoder_index)  # (batch_size * num_beams, cur_len, vocab_size)
            ...
            # before here is logits

            scores = F.log_softmax(next_token_logits, dim=-1)  # (batch_size * num_beams, vocab_size)

            # after here is log prob
            ...
            if do_sample:
                ...
                # add previous seq score, note that log(prob_token1)+log(prob_token2) is equivalent to prob_token1 * prob_token2, 
                # so the score is the likelihook of whole seq
                ...
                # find 2 * num_beams candiate for a batch, the number is because every hyp may finalized with EOS as next_token, 
                # so at most there would be num_beam hyp finished, but we still need at most additional num_beam hpy for following search
            ...
            # generate from which beam, which is unique across whole batch
        ...
        if model_kwargs.pop('output_scores', False):
    ...
    def add(self, hyp, sum_logprobs):
        ...
        if len(self) < self.num_beams or score > self.worst_score:
            ...
            if len(self) > self.num_beams:
                # already exceed max beam, removed the lowest one
                sorted_scores = sorted([(s, idx) for idx, (s, _) in enumerate(self.beams)])
                del self.beams[sorted_scores[0][1]]
                self.worst_score = sorted_scores[1][0]
            else:
                # less than max beam, just update worst score
