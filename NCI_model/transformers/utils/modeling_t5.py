...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        decode_vocab_size = getattr(config, "decode_vocab_size", None)
        tie_decode_embedding = getattr(config, "tie_decode_embedding", None)
        decode_depth = getattr(config, "decode_depth", None)
        if decode_embedding:
            assert decode_vocab_size is not None
            self.decode_embedding = nn.Embedding(decode_vocab_size + 2, config.d_model)
        else:
            self.decode_embedding = self.shared
        ...
        decoder_config.is_encoder_decoder = False
        decoder_config.num_layers = config.num_decoder_layers
        if hierarchic_decode:
            assert decode_depth is not None
            self.decoder = self.decoder = HierarchicT5Stack(decoder_config, self.decode_embedding, depth=decode_depth)
        else:
            self.decoder = T5Stack(decoder_config, self.decode_embedding)

        if decode_embedding and decode_vocab_size:
            self.lm_head = nn.Linear(config.d_model, decode_vocab_size + 2, bias=False)
        else:
            self.lm_head = nn.Linear(config.d_model, config.vocab_size, bias=False)

        if tie_decode_embedding:
            self._tie_or_clone_weights(self.lm_head, self.decode_embedding)
    ...
    def forward(
        ...
        # print(f"decoder_outputs[0]: {decoder_outputs[0].shape}")
